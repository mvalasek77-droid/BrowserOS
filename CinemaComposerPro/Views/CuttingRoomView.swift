...
struct CuttingRoomView: View {
    ...
    var body: some View {
        if entitlements.isPro || entitlements.isDemoActive {
            VStack(spacing: 0) {
                if entitlements.isDemoActive && !entitlements.isPro {
                    DemoBanner()
                }
                // What Pro unlocks is the magnetic editor: primary storyline,
                // connected lanes, all four trims, retiming and FCPXML.
                CuttingRoomProView()
            }
        } else {
...
/// Shown above the cutting room while a demo cut is on the table.
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button {
                ...
                Text("Exit demo").font(.caption.bold())
            }
...
final class DemoSession: ObservableObject {
    ...
    /// The magnetic cut is stashed alongside the track one, since the cutting
    /// room edits that and the demo must not stand on the user's real work.
    @Published var savedCut: MagneticTimeline?
    private var isActive = false

    /// Leaving the demo tears down the cutting room, whose `onDisappear` would
    /// otherwise commit the demo cut straight over the work we just restored.
    ...
    func enter(model: ProductionViewModel) {
        ...
        savedCut = model.cut
        let demo = DemoSequence.build()
        model.timeline = demo
        model.cut = MagneticTimeline(importing: demo)
