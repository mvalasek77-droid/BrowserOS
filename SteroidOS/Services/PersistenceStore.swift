...
class BookmarkStore {
    ...
    private let key = "steroidos_bookmarks"
    ...
    func load() -> [Bookmark] {
        ...
        do {
            ...
            ErrorLog.log("Failed to load bookmarks: \(error)")
...
class HistoryStore {
    private let defaults = UserDefaults.standard
    private let key = "steroidos_history"
    ...
    func load() -> [HistoryEntry] {
        ...
        do {
            ...
            ErrorLog.log("Failed to load history: \(error)")
            return []
        }
    }
...
class SettingsStore {
    private let defaults = UserDefaults.standard
    private let key = "steroidos_settings"
    ...
    func load() -> BrowserSettings {
        ...
        do {
            ...
            ErrorLog.log("Failed to load settings: \(error)")
...
// MARK: - Persistent Storage for Tabs

class TabStore {
    private let defaults = UserDefaults.standard
    private let key = "steroidos_tabs"
    
    func load() -> [BrowserTab] {
        guard let data = defaults.data(forKey: key) else { return [] }
        do {
            return try JSONDecoder().decode([BrowserTab].self, from: data)
        } catch {
            ErrorLog.log("Failed to load tabs: \(error)")
            return []
        }
    }
    
    func save(_ tabs: [BrowserTab]) {
        do {
            let data = try JSONEncoder().encode(tabs)
            defaults.set(data, forKey: key)
        } catch {
            ErrorLog.log("Failed to save tabs: \(error)")
        }
    }
}