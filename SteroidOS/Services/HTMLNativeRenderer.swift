...
actor HTMLNativeRenderer {
    ...
    private func decodeEntities(_ str: String) -> String {
        ...
        // Numeric entities — decode to their Unicode characters instead of stripping
        if let regex = try? NSRegularExpression(pattern: "&#(\\d+);", options: []) {
            var decoded = ""
            var lastEnd = result.startIndex
            regex.enumerateMatches(in: result, options: [], range: NSRange(result.startIndex..., in: result)) { match, _, _ in
                guard let match = match,
                      let wholeRange = Range(match.range, in: result),
                      let numberRange = Range(match.range(at: 1), in: result) else { return }
                decoded += String(result[lastEnd..<wholeRange.lowerBound])
                let numStr = String(result[numberRange])
                if let code = UInt32(numStr), let scalar = UnicodeScalar(code) {
                    decoded += String(scalar)
                }
                lastEnd = wholeRange.upperBound
            }
            decoded += String(result[lastEnd...])
            result = decoded
        }
        return result
    }
